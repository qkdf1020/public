...
def tesseract(image : str):
    """
    Input
        1) image (str) :
            인식하고 싶은 이미지
    """
    print(f"'{os.path.basename(image)}' 파일의 결과는 다음과 같습니다.\n")
    ...
    for i, plate_img in enumerate(plate_imgs):
        ...
        # 번호판 패턴: 숫자 2에서 3개, 한글 문자 1글자, 숫자 4개
        pattern = r'\d{2,3}[가-힣]\d{4}'

        # 번호판 문자열에서 패턴과 일치하는 부분 찾기
        matches = re.findall(pattern, result_chars)

        # 결과 출력
        if matches:
            # 첫 번째 일치하는 패턴을 선택하거나 모든 일치 패턴을 사용할 수 있습니다.
            matched_pattern = matches[0]  # 첫 번째 패턴 선택
            print("인식된 번호판 패턴 :", matched_pattern)
    ...
    return matched_pattern, x_coord, y_coord 